...
class ShareFormationPattern(Node):
    def __init__(self):
        ...
        self.formation_name = self.get_parameter('formation_name').get_parameter_value().string_array_value
        self.formation_id = self.get_parameter('formation_id').get_parameter_value().integer_array_value
        ...
        self.opinion = int(np.random.choice(self.formation_id))
    ...
    def cal_target_point(self, id):
        ...
        follower_ids = [int(robot_id[-1]) for robot_id in self.formation_name if int(robot_id[-1]) != self.leader_id]  # 获取跟随者的ID列表
...
def main(args=None):
    ...
    epuck_controller = ShareFormationPattern()
